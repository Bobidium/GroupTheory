...
**Proof**: We shall prove that the elements involved one cycle in the cycle decomposition forms an orbit under the group action of $S_n$ on $A = \{1, 2, ..., n\}$. In other words, if a permutation can be decomposed into $(12)(34)$, then the orbits of this permutation are precisely $\{1, 2\}$ and $\{3, 4\}$. The way we approach this is by using the group structure of $S_n$. For any $\sigma \in S_n$, we can consider the subgroup generated by $\sigma$, i.e. $\langle \sigma \rangle$. When this subgroup acts on $A$, it produces distinct orbits $\mathcal{O}_i$. Let $\mathcal{O}$ be one of these orbits and let $x \in \mathcal{O}$. From the proof of the Orbit-Stabilizer Theorem, we know that the function $f$ that maps $\sigma(x) \to \sigma G_x$ is a bijection. 
